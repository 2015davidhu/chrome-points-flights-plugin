...
type DiscoveryRequest = Pick<
  AwardSearchRequest,
  "origin" | "destination" | "startDate" | "endDate" | "cabin"
>;

function queryKey(input: DiscoveryRequest) {
...
type AwardDiscoveryServiceOptions = {
  provider: AwardSearchProvider;
  cacheStore: CacheStore;
};

export type AwardDiscoveryResult = {
...
export class AwardDiscoveryService {
  private readonly provider: AwardSearchProvider;

  private readonly cacheStore: CacheStore;

  constructor({ provider, cacheStore }: AwardDiscoveryServiceOptions) {
    this.provider = provider;
    this.cacheStore = cacheStore;
  }

  private async searchCandidates(input: DiscoveryRequest) {
    ...
    const candidates = await this.provider.searchCandidates({
      ...input,
      walletBalances: {
        amex_mr: 0,
        chase_ur: 0,
  ...
  async discover(input: DiscoveryRequest): Promise<AwardDiscoveryResult> {
